...
def cameratoworld_torch(corners, position, rotation):
    ...
    print("listen up ",rotation," you cheeky cunt")
...
def draw_3d_cube(frame,
    ...
    for p1, p2 in projpoints:
        if not math.isnan(p1[0]) and not math.isnan(p2[0]) and not math.isnan(p1[1]) and not math.isnan(p2[1]) and p1[0] < 10000 and p2[0] < 10000 and p1[0] > 0 and p2[0] > 0:
            print("check this:",(int(p1[0]), int(p1[1])), 
                    (int(p2[0]), int(p2[1])),
                    line_color,
                    line_width)
            cv2.line(frame, 
