...
import ss_mono_3d_tracking as tracky
...
if __name__ == "__main__":
    ...
    if args.input:
        ...
        for vis, preds in tqdm.tqdm(demo.run_on_video(cam)):
            ...
            something, box_info = tracky.turn_preds_to_data(frame_data, vis, 0, dataset)
            tense_frame = torch.Tensor(np.moveaxis(np.expand_dims(vis,0),3,1))
            model, tracking_list = estims.setup()
            model.eval()
            tracking_list = estims.ss_box_me(model, tense_frame, box_info, tracking_list)
            mot_tracker = tracky.setup_tracker(dataset)
            #print(type(tracking_list[0]), len(tracking_list))
            frames_anno, frames_hypo = tracky.update_tracker(mot_tracker, tracking_list[0], vis, count)
            print(frames_anno, frames_hypo)
            plot_tracking.plot_shit(frames_anno, frames_hypo, birds_eye, draw2d, draw3d, "Webcam_boii", vis)
            #cv2.namedWindow(WINDOW_NAME, cv2.WINDOW_NORMAL)
