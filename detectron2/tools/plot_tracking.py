...
def plot_shit(info_gt, info_pd, draw_bev, draw_2d, draw_3d, session_name, rawimg):
    if draw_bev: print("BEV: {}".format(session_name))
    if draw_2d or draw_3d: print("3D: {}".format(session_name))
    ...
    for n_frame, (pd_boxes, gt_boxes) in enumerate(
        ...
        cam_calib = np.array([[935.3074360871937, 0, 960, 0],[0, 935.3074360871937, 540, 0],[0, 0, 1, 0]])
        cam_pose = tu.Pose(cam_loc, cam_rot)
        ...
        for i, hypo in enumerate(pd_boxes['hypotheses']):
            ...
            box_pd = np.array(hypo[box_key]).astype(int)
        ...
        elif draw_3d or draw_2d:
            #draw_img = rawimg[:, :, ::-1]

            #fig = plt.figure(figsize=(18, 9), dpi=50)
            #plt.imshow(draw_img)
            key = 0
            cv2.imshow('frame', cv2.resize(rawimg, (resW, resH)))
