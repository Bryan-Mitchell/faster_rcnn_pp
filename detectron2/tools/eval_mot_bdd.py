...
def parse_args():
    ...
    'lstmocc_age20_aff0.1_hit0_100m_803', type=str)
...
class TrackEvaluation:
    ...
    def eval_app(self):
        ...
        for i_s, (seq_gt, seq_pd) in enumerate(tqdm(
            ...
            print(np.all([item['hypotheses'] == [] for item in seq_pd['frames']]))
            print(np.all([item['annotations'] == [] for item in seq_gt['frames']]))
            result[i_s] = self.eval_parallel(seq_gt, seq_pd)

        for res_key in result:
            this_abs_stats, this_relative_stats = result[res_key]
            #print(this_abs_stats, this_relative_stats)
...
def main():
    ...
    #print(np.all([item['hypotheses'] == [] for item in seq_pd_list[0]['frames']]))
    te = TrackEvaluation(
            seq_gt_list, 
            seq_pd_list,
            args.pd_path, 
