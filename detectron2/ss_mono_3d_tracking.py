...
def turn_preds_to_data(frame_pd, img, idx, dataset):
    n_box_limit = 20
    ...
    cam_rot = np.array([0.0, 0.0, 0.0])#np.array(frame['extrinsics']['rotation'])
    cam_loc = np.array([0.0,0.0,0.0])#np.array(frame['extrinsics']['location'])
...
def main():
    ...
    #data_iter, fasterRCNN, args, imdb, im_vars, start = detector.test_net()
    ret = True
    draw3d = False
    draw2d = True
    birds_eye = True
    ...
    while ret:
        ...
        frame_old = frame
        #all_boxes = detector.run_single(fasterRCNN, frame, args.class_agnostic, True, True, imdb, im_vars, start)
        print(all_boxes, "test")
