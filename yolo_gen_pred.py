...
def parse_args():
    ...
    parser.add_argument('--dset', choices=['gta', 'kitti'],
                        help='Generate GTA or KITTI dataset')
    #parser.add_argument('split', choices=['train', 'val', 'test'], help='Generate GTA train, validation or test dataset', default="test")
...
if args.dset == 'gta':
    ...
    det_name = os.path.join(DATASET.PATH,
                            'gta_{}_detections.pkl'.format(split))
    save_name = os.path.join(DATASET.PATH,
                            'gta_{}_list.json'.format(split))
    DATASET.LABEL_PATH = DATASET.LABEL_PATH.replace('train', split)
    DATASET.PRED_PATH = DATASET.PRED_PATH.replace('train', split)
    DATASET.IM_PATH = DATASET.IM_PATH.replace('train', split)
else:
    ...
    if args.kitti_task == 'detect':
        ...
        det_name = os.path.join(DATASET.PATH,
                            'kitti_{}_obj_detections.pkl'.format(
                                split))
        save_name = os.path.join(DATASET.PATH,
                            'kitti_{}_obj_list.json'.format(split))
        ...
        det_name = os.path.join(DATASET.PATH,
                            'kitti_{}_trk_detections.pkl'.format(
                                split))
        save_name = os.path.join(DATASET.PATH,
                             'kitti_{}_trk_list.json'.format(split))

    DATASET.LABEL_PATH = DATASET.LABEL_PATH.replace('train', split)
    DATASET.PRED_PATH = DATASET.PRED_PATH.replace('train', split)
    DATASET.IM_PATH = DATASET.IM_PATH.replace('train', split)
    assert split in ['train', 'test'], "KITTI has no validation set"
...
def ss_pred_gen(det_result, idx):
    seq_data = []
    frame_data = {}
    prediction_data = []
    if len(det_result) != 0:
        match_box = det_result
        for box in match_box:
            ...
            if len(box) == 5:
                ...
                prediction['box2d']['confidence'] = 69.69
            # 3D box projected center
            prediction['box3d']['xc'] = int(box[0]+box[2]/2)
            prediction['box3d']['yc'] = int(box[1]+box[3]/2)
...
class Dataset():
    ...
    def __init__(self):
        ...
        pattern = 'final.json' if args.dset == 'gta' else '.json'
