...
#Make sure to remove the formatting if youre not using gta dataset
...
#  for i in range(num_images):
      ...
      if cfg.TEST.BBOX_REG:
          ...
          # Optionally normalize targets by a precomputed mean and stdev
            if args.class_agnostic:
                ...
                center_deltas = center_deltas.view(1, -1, 2 * len(imdb.classes))

          pred_boxes = bbox_transform_inv(boxes, box_deltas, 1)
          #NEED TO ADD CENTER_TRANSFOR_INV
      ...
      for j in xrange(1, imdb.num_classes):
          ...
          if inds.numel() > 0:
            ...
            #keep = torch.arange(cls_dets.shape[0]) 
            cls_dets = cls_dets[keep.view(-1).long()]
            cls_dets_with_center = cls_dets_with_center[keep.view(-1).long()]
            if vis:
              im2show = vis_detections(im2show, imdb.classes[j], cls_dets.cpu().numpy(), 0.3)
              #im2show = vis_detections(im2show, imdb.classes[j], cls_dets_with_center.cpu().numpy(), gt_boxes.cpu().numpy(), fixed_center.cpu().numpy(), num_boxes.cpu().numpy(), 0.8)
