...
def run_training(model, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for phase in phases:
            ...
            }, args.dset == 'kitti', args.session, args.check_freq)
...
def test_model(model, args):
    ...
    #dataset = Dataset(args.json_path, 'test',args.data_split,args.set == 'kitti',args.percent,args.is_tracking,args.is_normalizing,args.n_box_limit)

    #print("Number of image to test: {}".format(dataset.__len__()))
    #print(args.json_path, args.data_split)
    # Data loading code
    #test_loader = DataLoader(dataset,batch_size=args.batch_size, shuffle=False,num_workers=args.workers, pin_memory=True, drop_last=True)
    model.eval()
    end = time.time()
 
    #image, box_info = next(iter(test_loader))
    #Change to function that estimates boxes for single image.
...
def ss_box_me(model, image, box_info, tracking_list):
    # measure data loading time
    with torch.no_grad():
        box_output, targets = model(image, box_info, torch.device("cuda"), 'test')
    #print("Step2")
    rois_gt, \
    dim_gt_, \
    alpha_gt_, \
